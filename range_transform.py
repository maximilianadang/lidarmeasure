...
def range_data(time_ps, counts, config):
    ...
    if not np.any(mask): raise ValueError('No positive-range bins in the configured plotting window')
...
def weight_counts(counts, ranges): return np.asarray(counts) / np.asarray(ranges) ** 4
